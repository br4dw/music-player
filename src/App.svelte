	...
	function changeDirectory() {
		...
		fs.readdir(config.path, async (error, data) => {
			...
			let filtered = data.filter((f) =>
				types.some((type) => f.endsWith(type))
			);
			...
			fs.writeFile(
				...
				JSON.stringify({ path: folderPath, files }),
				(writeError) => {
					if (writeError) {
...
<nav class="topnav">
	<div class="left">
		<div class="nav-item accent">Mystic</div>
	</div>
	<div class="right">
		<div class="nav-item">
			<!-- Folder import -->
...
<div class="centered">
	<div class="spacer">
		...
		<h2 class="subtitle">{files.length < 1 ? "No suitable files have been found or no folder has been selected..." : ""}
		</h2>
		<!-- Files -->
		<div class="files">
			{#if directoryChanging}
				<div class="file">
					<div class="loading-files">
						<h2>Loading files.. Please wait</h2>
					</div>
					{#if currentFile}
			...
			{:else}
				{#each files as file}
					<div class="file">
						...
						<img id={file.raw + "-art"} class="album-art thumbnail" src={file.thumb ? file.thumb : thumbnail} alt="Art"/>
		...
		<div class="duration-container">
			<div class="progress-container">
				...
				<input
					...
					class="slider" id="slider" type="range" value="0" min="0" max="100" step=".1"
				/>
...
<style scoped>
	:root {
		--player-height: 120px;
		--accent-primary: #b94ae2;
		--accent-secondary: #af1ef1;
		--accent-primary-filter: invert(46%) sepia(48%) saturate(6383%) hue-rotate(261deg) brightness(93%) contrast(89%);
		--accent-secondary-filter: invert(23%) sepia(52%) saturate(5026%) hue-rotate(273deg) brightness(92%) contrast(111%);
		--bar: rgba(233, 121, 255, 0.9);
		--bar-overlay: rgb(152, 67, 231);
		--background: #1f2024;
		--background-darker: #131316;
		--title: rgb(167, 166, 180);
		--subtitle: rgb(73, 73, 77);
		--accent-title: var(--accent-primary);
		--accent-subtitle: var(--accent-secondary);
	}
	.topnav {
	...
	.icon {
		...
		cursor: pointer;
		float: left;
		filter: var(--accent-primary-filter);
	}
	.icon:hover {
		filter: var(--accent-secondary-filter);
	}
	.thumbnail {
		float: left;
	}
	...
	.control-icon {
		filter: invert(90%) sepia(7%) saturate(169%) hue-rotate(196deg)
			brightness(106%) contrast(91%) !important;
	}
	.title-icon {
		width: 35px;
		height: 35px;
	...
	.control-icon:hover {
		filter: invert(93%) sepia(17%) saturate(121%) hue-rotate(201deg)
